...
$id_agent = $_SESSION['id_agent'];
    ...
    $statusd = $_POST['setstatus'];
    $statuscomment = $_POST['statuscomment'];
    $process = '20%';
    $datecom = $_POST['datecomment'];

if ($statusd === 'DOUBLE_ORDER') {
    $statusd = 16;
} else if ($statusd === 'NOT_ORDERED') {
    $statusd = 15;
} else if ($statusd === 'SPAM') {
    $statusd = 17;
}

$statusprocces = 19;

    $updatestatus = "UPDATE tb_customer SET id_statusd = '$statusd',id_statusf = '$statusprocces', operator_comment = '$statuscomment', date_comment = '$datecom'  WHERE id_customer = '$id_customer'";
    ...
    if(mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            VALUES ('{$row['note']}','{$row['id_customer']}','{$row['first_name']}','{$row['last_name']}','{$row['phone']}','{$row['country']}','{$row['timezone']}','{$row['addrest_list']}','{$row['state']}','{$row['city']}','{$row['zip']}','{$row['shipper']}','{$row['order_date']}','{$row['gender']}','{$row['email']}','{$row['age']}','{$row['height']}','{$row['Chronic_disease']}','{$row['married']}','{$row['children']}','','','','{$row['id_statusd']}','','{$row['id_agent']}','{$row['order_id']}','{$row['operator_comment']}','{$row['date_comment']}','{$row['promo']}','{$row['nama_product']}','{$row['description']}','{$row['package']}','{$row['shipment_type']}','{$row['amount']}','{$row['price']}','{$row['total']}','{$row['bank_payment']}')
            ...
            $inserthistori = "INSERT INTO history_agent (note, id_customer, first_name, last_name, phone, country, timezone, addrest_list, state, city, zip, shipper, order_date, gender, email, age, height, Chronic_disease, married, children, id_status, id_statusb, id_statusc, id_statusd, id_statusf, id_agent, order_id, operator_comment, operator_comment2, operator_comment3,operator_comment4, date_comment, date_comment2, date_comment3, date_comment4, promo, nama_product, description, package, shipment_type, amount, price, total, bank_payment)
             VALUES ('{$row['note']}','{$row['id_customer']}','{$row['first_name']}','{$row['last_name']}','{$row['phone']}','{$row['country']}','{$row['timezone']}','{$row['addrest_list']}','{$row['state']}','{$row['city']}','{$row['zip']}','{$row['shipper']}','{$row['order_date']}','{$row['gender']}','{$row['email']}','{$row['age']}','{$row['height']}','{$row['Chronic_disease']}','{$row['married']}','{$row['children']}','','','','{$row['id_statusd']}','','{$row['id_agent']}','{$row['order_id']}','{$row['operator_comment']}','{$row['note']}','{$row['note']}','{$row['note']}','{$row['date_comment']}','{$row['date_comment']}','{$row['date_comment']}','{$row['date_comment']}','{$row['promo']}','{$row['nama_product']}','{$row['description']}','{$row['package']}','{$row['shipment_type']}','{$row['amount']}','{$row['price']}','{$row['total']}','{$row['bank_payment']}')";                
