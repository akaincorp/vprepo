            ...
            <div class="card card-primary card-outline card-outline-tabs p-3">
              <div class="card-header p-0 border-bottom-0">
                <ul class="nav nav-tabs" id="custom-tabs-four-tab" role="tablist">
                  ...
                  <li class="nav-item">
                    <a class="nav-link active" href="groupreport.php"  aria-selected="true">Group Report</a>
                  </li>
                  <li class="nav-item">
...
<?php
$leader = "SELECT * FROM stat_leader JOIN tb_leader ON stat_leader.id_leader = tb_leader.id_leader"; 
$dataleader = mysqli_query($con, $leader);
      ...
      <label for="datePicker">Date:</label>
        <input type="date" name="datePicker" id="datePicker" onchange="sortByDate()">
            ...
            <table class="table table-bordered table-hover mt-3" id="tb1">
                ...
                <tbody>
                <?php 
                while ($d = mysqli_fetch_assoc($dataleader)) {  ?>
                    <tr>
                        <td><?php echo $d['username'] ?></td>
                        <td><?php echo $d['turn_over'] ?></td>
                        <td><?php echo $d['approved_amount'] ?></td>
                        <td><?php echo $d['gross_com'] ?></td>
                        <td><?php echo $d['reject'] ?></td>
                        <td><?php echo $d['aff_payout'] ?></td>
                        <td><?php echo $d['last_date'] ?> </td>
                    </tr>
                    <?php } ?>
                </tbody>
            </table>
...
<script>
  ...
  $.ajax({
    type: 'GET',
    url: 'historyselect.php',
    data: { date: selectedDate },
    success: function(response) {
        let rawdata = JSON.parse(response);

        // Simpan referensi ke elemen <tbody>
        const tableBody = $('#tb1 tbody').empty();

        // Loop melalui data dan tambahkan baris ke elemen <tbody>
        rawdata.forEach(d => {
            const newRow = `
                <tr>
                    <td>${d.username}</td>
                    <td>${d.turn_over}</td>
                    <td>${d.approved_amount}</td>
                    <td>${d.gross_com}</td>
                    <td>${d.reject}</td>
                    <td>${d.aff_payout}</td>
                    <td>${d.last_date}</td>
