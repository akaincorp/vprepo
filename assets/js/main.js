...
function loadCallOptions() {
  if (window.localStorage) {
    ...
    txtCallStatus.innerHTML = '<i>Configuration ' + (bDisableVideo ? 'success' : 'failed') + '</i>';
  }
}
    ...
    if (s_type == 'call-screenshare') {
      ...
      if (!location.protocol.match('https')) {
        if (confirm("Screen sharing requires https://. Do you want to be redirected?")) {
          sipUnRegister();
          window.location = 'http://localhost:3000/call.html';
        }
        return;
      }
    }
...
function uiOnConnectionEvent(b_connected, b_connecting) { // should be enum: connecting, connected, terminating, terminated
  ...
  btnMute.disabled = !oSipSessionCall;
  btnKeyPad.disabled = !oSipSessionCall;
}
...
function startTimer() {
  ...
  // Inisialisasi startTime di sini
    ...
    case 'stopping': case 'stopped': case 'failed_to_start': case 'failed_to_stop':
      {
        ...
        txtCallStatus.innerHTML = bFailure ? "<b>" + e.description + "</b>" : "<i>Disconnected</i>";
        break;
      }
